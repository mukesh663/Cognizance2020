...
def main():
    ...
    if a==('y'):
        ...
        for i in range(0,b):
            ...
            c.execute( "INSERT INTO StudentList VALUES(?)", (name,))
            conn.commit()
    else:
        print('ok') 
    ...
    if inp=='y':
      ...
      for message in messages[0:d]:
         ...
         for i in range(0,len(name)):
             if cont[i]['name']=='Subject':                  # getting the From address of the mail from headers field
                 content=name[i]['value']
                 break            
         values.append((mailname,str(timestamp),content))
      c.execute("CREATE TABLE IF NOT EXISTS MailList(Name TEXT,Timestamp DATETIME, Content TEXT)")
      for row in values:
         print(row)
         c.execute( "INSERT INTO MailList VALUES(?, ?, ?)", row)
         conn.commit()
    else:
        print('ok')
